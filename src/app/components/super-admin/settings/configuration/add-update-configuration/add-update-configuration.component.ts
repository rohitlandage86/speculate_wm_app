...
export class AddUpdateConfigurationComponent implements OnInit{
  ...
  updateConfiguration() {
    ...
    console.log(this.configurationForm.valid);
    if (this.configurationForm.valid) {
      console.log("bnskjd", this.configurationForm.value);
      this._superAdminService.editConfiguration(this.configuration_Id, data).subscribe({
        next: (res: any) => {
          if (res.status == 200) {
            this._toastrService.success(res.message);
            this.router.navigate(['/super-admin', { outlets: { super_Menu: 'configuration' } }]);
          } else {
