...
data:any={};
  ...
  updateEvent() {
    ...
    if (this.eventForm.valid) {
      console.log("bnskjd", this.eventForm.value);
      this._superAdminService.editBettingEventType(this.event_Id, data).subscribe({
        next: (res: any) => {
          if (res.status == 200) {
            this._toastrService.success(res.message);
            this.router.navigate(['/super-admin', { outlets: { super_Menu: 'event' } }]);
          } else {
