...
export class AddUpdateMarketComponent implements OnInit{
  ...
  updateMarket() {
    ...
    console.log(this.marketForm.valid);
    if (this.marketForm.valid) {
      console.log("bnskjd", this.marketForm.value);
      this._superAdminService.editBettingMarketType(this.market_Id, data).subscribe({
        next: (res: any) => {
          if (res.status == 200) {
            this._toastrService.success(res.message);
            this.router.navigate(['/super-admin', { outlets: { super_Menu: 'market' } }]);
          } else {
